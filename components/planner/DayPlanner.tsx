...
export const DayPlanner: React.FC<DayPlannerProps> = ({ selectedDate }) => {
  ...
  const getCurrentTimePosition = () => {
    ...
    const minutesIntoHour = minutes - (hour * 60);

    // Get base offset for the hour + position within that hour
    const baseOffset = getHourOffset(hour);
    const withinHourOffset = (minutesIntoHour / 60) * hourHeights[hour];

    return baseOffset + withinHourOffset;
  };
  ...
  const getSlotHeight = (slot: PlannerSlot) => {
    // For very small tasks (<5 min), use thin line
    if (slot.durationMinutes < 5) {
      return 4; // Thin line
    }
    // Minimum height of 20 minutes equivalent for regular tasks
    const minHeight = 20;
  ...
  // Calculate dynamic hour heights based on slot density
  const getHourHeights = () => {
    const hourHeights: number[] = new Array(24).fill(HOUR_HEIGHT);

    // Count slots per hour
    const slotsPerHour: number[] = new Array(24).fill(0);

    const durationPerHour: number[] = new Array(24).fill(0);
    ...
    slots.forEach(slot => {
      ...
      while (current < end) {
        ...
        // End of the current hour or slot end
    ...
    // Expand hours with many slots (more than 2)
    slotsPerHour.forEach((count, hour) => {
      if (count > 2 && durationPerHour[hour] > 20) {
        hourHeights[hour] = HOUR_HEIGHT + (count - 2) * 15; // Add 15px per extra slot
      }
    });
  ...
  const hourHeights = getHourHeights();

  // Calculate cumulative positions for dynamic heights
  const getHourOffset = (hour: number) => {
    let offset = 0;
    ...
    return (
      <View style={styles.timeLabels}>
        {hours.map((hour, index) => (
          <View key={index} style={[styles.timeLabel, { height: hourHeights[index] }]}>
  ...
  const renderSlots = () => {
    return slots.map(slot => {
      ...
      const isSmallTask = slot.durationMinutes < 5;
        ...
        <TouchableOpacity
          ...
          style={[
            styles.slot,
            {
              ...
              padding: isSmallTask ? 0 : 8, // No padding for small tasks
              borderRadius: isSmallTask ? 2 : 8, // Smaller radius for lines
            },
          ]}
          ...
          {!isSmallTask && (
            <View style={styles.slotContent}>
              <Text style={styles.slotTaskName} numberOfLines={1}>
                {String(slot.taskName ?? '')}
              </Text>
              <Text style={styles.slotDuration}>
                {String(formatDuration(slot.durationMinutes * 60) ?? '')}
              </Text>
