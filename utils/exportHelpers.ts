...
export const exportToCSV = async (data: ExportData): Promise<string> => {
  ...
  // CSV Headers
  const headers = ['Date', 'Task Name', 'Start Time', 'End Time', 'Duration (minutes)', 'Status', 'Notes'];
    ...
    .map(entry => {
      ...
      return `
        <tr>
          ...
          <td style="padding: 6px; border-bottom: 1px solid #eee; font-size: 10px; color: #666; font-style: italic; max-width: 150px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">${
            entry.notes || ''
          }</td>
        </tr>
      `;
